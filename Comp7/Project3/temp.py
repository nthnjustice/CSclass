...
def readInClasses(inputfile):
    f = open(inputfile, "r")
    classes = {}
    tempId = "" # declare a temporary variable to store an id
    
    for line in f:
        # Check to see if the first line indicates an id
        if line[0] == ">":
            tempId = line # assign the line to the temporary variable
            #### 
            #### manipulate the line so the id is in the proper format
            ####
            classes[tempId] = "" # add an entry with the temporary Id
        else:
            # if the line is not an id, but rather a classification breakdown
            # then store that line into the id for the line directly above it
            # the id directly above it is stored in the variable tempID
            # tempID will update to the new id as soon as this classification
            # breakdown is stored in the dictionary
            classes[tempId] = line
