...
void FrequencyTable::insert(string word)
{
  ...
  if(in(word)){
    temp = whatnode(word);//idenifies the node
    set_freq(temp, (frequency(word) +1));//changes the frequency
  }
  else{
    addnode(word);
  }
...
int FrequencyTable::frequency(string word)
{
  ...
  while(temp != empty()){
    if(word == temp->wfpair.word){//means the word is in the list
...
int FrequencyTable::size()
{
  ...
  return counter;//size of the list
}
...
void FrequencyTable::get(int n, string *ptrToWord, int *ptrToFrequency)
{
  ...
  *ptrToFrequency = frequency(*ptrToWord);//points to the frequency of the word
}
...
{
  while(!isEmpty(list)){
    list = deleteHead(list);//kills the list
  }
}
